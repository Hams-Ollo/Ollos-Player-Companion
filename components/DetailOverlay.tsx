...
const DetailOverlay: React.FC<DetailOverlayProps> = ({ isOpen, type, onClose, title, color, children }) => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`pt-safe-area flex items-center justify-between p-4 border-b ${headerClass} shadow-lg shrink-0`}>
         <h2 className="text-2xl font-display font-bold tracking-wide">{title}</h2>
         <button 
           onClick={onClose}
      ...
      {/* Scrollable Content */}
      <div className={`flex-grow overflow-y-auto overflow-x-hidden p-4 pb-24 animate-in slide-in-from-bottom-10 duration-300 no-scrollbar`}>
